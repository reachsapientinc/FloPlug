...
echo "📂 Fetching Sandbox Keys from floplug-sb folder..."
cp floplug-sb/.env.sb frontend/.env.sb
cp floplug-sb/.env.functions functions/.env
trap 'rm -f frontend/.env.sb functions/.env; echo "🧹 Cleaned up."' EXIT
...
echo "🏗️ Starting Sandbox Build..."
cd frontend
if npx vite build --mode sb; then

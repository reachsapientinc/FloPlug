...
echo "📂 Fetching Stage Keys from floplug-stage folder..."
cp floplug-stage/.env.stage frontend/.env.stage
cp floplug-stage/.env.functions functions/.env
trap 'rm -f frontend/.env.stage functions/.env; echo "🧹 Cleaned up."' EXIT
...
echo "🏗️ Starting Stage Build..."
cd frontend
if npx vite build --mode stage; then

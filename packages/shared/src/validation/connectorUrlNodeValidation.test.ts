import {
  ...
  validateConnectorUrlNode,
  type ConnectorUrlToken,
} from '@floplug/shared';
...
assert(
  findUnresolvedUrlPlaceholders('https://x.com/{Tenant}/v1').includes('Tenant'),
  'single-brace segment',
);
assert(
  findUnresolvedUrlPlaceholders('https://x.com/{{tenant}}/api').includes('tenant'),
  'mustache segment',
);
assert(
  findUnresolvedUrlPlaceholders('https://x.com/acme/v1').length === 0,
  'fully resolved url',
);

const tokens: ConnectorUrlToken[] = [

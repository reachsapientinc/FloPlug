...
type TokType =
  ...
  | 'IDENT'   // plain word — could be a path segment, function name, keyword
  | 'LPAREN' | 'RPAREN' | 'COMMA'
  | 'OP'      // +  -  *  /  %  ==  !=  <  <=  >  >=  &&  ||  !
  | 'DOT'     // path separator
  | 'EOF';
...
function tokenize(src: string): Token[] {
  ...
  while (i < len) {
    ...
    if (src[i] === '(') { tokens.push({ type: 'LPAREN', value: '(', pos: i++ }); continue; }
    if (src[i] === ')') { tokens.push({ type: 'RPAREN', value: ')', pos: i++ }); continue; }
    if (src[i] === ',') { tokens.push({ type: 'COMMA',  value: ',', pos: i++ }); continue; }
    if (src[i] === '.') { tokens.push({ type: 'DOT',    value: '.', pos: i++ }); continue; }

    // Identifier / keyword (letters, digits, underscore)
    if (/[a-zA-Z_$]/.test(src[i])) {
      let s = '';
      while (i < len && /[a-zA-Z0-9_$]/.test(src[i])) { s += src[i++]; }
      if (s === 'true' || s === 'false') tokens.push({ type: 'BOOL', value: s, pos: i });
      else if (s === 'null')             tokens.push({ type: 'NULL', value: s, pos: i });
      else                              tokens.push({ type: 'IDENT', value: s, pos: i });
      continue;
    }
...
type AstNode =
  | { kind: 'Literal';  value: FloValue }
  | { kind: 'Path';     parts: string[] }            // cStream.a.b
...
class Parser {
  ...
  private expect(type: TokType): Token {
    ...
    if (t.type !== type) throw new Error(`[FloExpression] Expected ${type} but got ${t.type} ('${t.value}')`);
    return t;
  }

  parse(): AstNode { const n = this.parseExpr(0); return n; }

  // Operator precedence table
  private prec(op: string): number {
    if (op === '||' || op.toUpperCase() === 'OR')  return 1;
    if (op === '&&' || op.toUpperCase() === 'AND') return 2;
  ...
  private parseExpr(minPrec: number): AstNode {
    ...
    while (true) {
      ...
      const isBinOp =
        (t.type === 'OP'   && this.prec(t.value) > minPrec) ||
  ...
  private parsePrimary(): AstNode {
    ...
    // Literals
    if (t.type === 'NUMBER') { this.consume(); return { kind: 'Literal', value: parseFloat(t.value) }; }
    if (t.type === 'STRING') { this.consume(); return { kind: 'Literal', value: t.value }; }
    if (t.type === 'BOOL')   { this.consume(); return { kind: 'Literal', value: t.value === 'true' }; }
    if (t.type === 'NULL')   { this.consume(); return { kind: 'Literal', value: null }; }
...
function resolvePath(parts: string[], ctx: EvalContext): FloValue {
...
type FnImpl = (args: FloValue[]) => FloValue;

function asStr(v: FloValue): string  { return v === null || v === undefined ? '' : String(v); }
function asNum(v: FloValue): number  { return Number(v); }
function asBool(v: FloValue): boolean { return Boolean(v); }
...
const FUNCTIONS: Record<string, FnImpl> = {
  ...
  trim(args) {
    ...
    if (side === 'l' || side === 'left')  return s.trimStart();
    if (side === 'r' || side === 'right') return s.trimEnd();
    return s.trim();
  },
  ...
  pad(args) {
    ...
    if (side === 'l' || side === 'left')  return s.padStart(size, ch);
  ...
  split(args) {
    ...
    if (args[2] !== undefined) return parts[asNum(args[2])] ?? null;
    return parts;
  },
  ...
  format(args) {
    ...
    if (p.includes('YYYY') || p.includes('MM') || p.includes('DD') ||
      ...
      const d = new Date(src as string | number);
      if (isNaN(d.getTime())) return asStr(src);
      return p
...
function evalAst(node: AstNode, ctx: EvalContext): FloValue {
  switch (node.kind) {
    case 'Literal':
      return node.value;
    ...
    case 'Call': {
      ...
      if (!fn) throw new Error(`[FloExpression] Unknown function: ${node.name}()`);
    ...
    case 'BinOp': {
      ...
      if (node.op === '&&') return asBool(l) ? evalAst(node.right, ctx) : false;
      if (node.op === '||') return asBool(l) ? l : evalAst(node.right, ctx);
      const r = evalAst(node.right, ctx);
      switch (node.op) {
        case '+':  return (typeof l === 'number' && typeof r === 'number') ? l + r : asStr(l) + asStr(r);
        case '-':  return asNum(l) - asNum(r);
        case '*':  return asNum(l) * asNum(r);
        case '/':  { const d = asNum(r); if (d === 0) throw new Error('[FloExpression] Division by zero'); return asNum(l) / d; }
        case '%':  return asNum(l) % asNum(r);
    ...
    case 'UnaryOp': {
      ...
      if (node.op === '-') return -asNum(v);
      if (node.op === '!') return !asBool(v);
 ...
 */
export function evalExpression(expr: string, ctx: EvalContext): FloValue {
 ...
 */
export function safeEvalExpression(expr: string, ctx: EvalContext, fallback: FloValue = undefined): FloValue {
  try {
...
export function looksLikeExpression(s: string): boolean {
  ...
  if (/\b(AND|OR|NOT)\b/i.test(t)) return true;
  if (/\b(true|false|null)\b/i.test(t)) return true;
  if (/&&|\|\||[+\-*/%<>=!]/.test(t)) return true;
...
export function structuralExpressionCheck(expr: string): string | null {
  ...
  if (depth > 0) return `Missing ${depth} closing ')'`;
 ...
 */
export function validateFloExpression(expr: string): FloExpressionValidation {
  const text = expr?.trim() ?? '';

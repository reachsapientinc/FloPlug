...
function fieldOptional(f: ParsedField | undefined): boolean {
  if (!f) return false;
  const mo = f.minOccurs;
  if (mo === '0' || mo === 0) return true;
  if (f.xsdType === 'object' && !f.required) return true;
  if (!f.required) return true;
  return false;
}

/** True when any mapped value or rule exists at or below prefix. */
export function isMappingBranchActivated(
  prefix: string,
  resolved: Record<string, unknown>,
  mappingRules: MappingRuleTarget[] = [],
): boolean {
  if (prefix in resolved && resolved[prefix] !== undefined && resolved[prefix] !== null) {
    return true;
  }
  const dot = `${prefix}.`;
  if (Object.keys(resolved).some(k => k.startsWith(dot))) return true;
...
function schemaByPath(inputSchema: ParsedField[]): Map<string, ParsedField> {
...
export function buildOptionalContainerPaths(inputSchema: ParsedField[]): Set<string> {
 ...
 */
export function isImplicitOptionalContainer(
  prefix: string,
  inputSchema: ParsedField[],
): boolean {
  const byPath = schemaByPath(inputSchema);
  const exact = byPath.get(prefix);
  if (exact && fieldOptional(exact)) return true;
  if (exact?.required === true && !fieldOptional(exact)) return false;
  ...
  for (const f of nested) {
    ...
    if (seg) childPrefixes.add(`${prefix}.${seg}`);
  }

  for (const cp of childPrefixes) {
    const child = byPath.get(cp);
    if (child && fieldOptional(child)) return true;
  }
...
function isOptionalContainerPrefix(
  prefix: string,
  inputSchema: ParsedField[],
  optionalPaths: Set<string>,
): boolean {
  if (optionalPaths.has(prefix)) return true;
...
export function isFieldEffectivelyRequired(
  ...
  for (const p of field.optionalAncestorPaths ?? []) {
    if (optionalPaths.has(p) || isImplicitOptionalContainer(p, inputSchema)) {
      if (!isMappingBranchActivated(p, resolved, mappingRules)) return false;
    }
  }

  for (const prefix of ancestorPrefixes(field.path)) {
    if (
      isOptionalContainerPrefix(prefix, inputSchema, optionalPaths)
...
export function listUnmappedRequiredFields(
  inputSchema: ParsedField[],
  resolved: Record<string, unknown>,
  mappingRules: MappingRuleTarget[] = [],
): string[] {

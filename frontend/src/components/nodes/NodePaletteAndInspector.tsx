      ...
      {isDynamic && (
        <>
          <input
            ...
            onChange={e => onChange({ value: e.target.value })}
          />
          <div style={{ fontSize: 9, color: '#39ff14', marginTop: 3, lineHeight: 1.5 }}>
            {binding.source === 'cStream' && (
              <>
                <strong style={{ color: '#f0f0f4' }}>cStream</strong> always refers to <code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>cStream.message</code> — the current payload.<br/>
                Leave blank → entire upstream payload.<br/>
                <code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>empId</code> → <code style={{ fontFamily: 'monospace', color: '#4f8ef7' }}>cStream.message.empId</code><br/>
                <code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>invoice.total</code> → nested dot-path within message.
              </>
            )}
            {binding.source === 'local'   && <><code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>varName</code> or <code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>nested.field</code> within local store</>}
            {binding.source === 'global'  && <><code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>varName</code> or <code style={{ fontFamily: 'monospace', color: '#f59e0b' }}>config.key</code> within global store</>}
          </div>

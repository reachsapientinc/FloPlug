 ...
 */
export declare const ERROR_HANDLE = "error";
/** Per-node override; inherit uses Start node floErrorDefaults. */
export type CatchErrorScope = 'inherit' | 'none' | 'self' | 'subtree';
export type ErrorPathDataSource = 'inherit' | 'cStreamAtCatcher' | 'cStreamAtError' | 'local' | 'global';
...
export interface ExecutionPathState {
    ...
    errorPropagation?: {
        from: string;
        to: string;
        action: 'raised' | 'caught' | 'propagated' | 'unhandled';
    }[];
}

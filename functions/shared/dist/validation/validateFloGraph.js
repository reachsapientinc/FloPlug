...
import { allLoopRegionNodeIds, collectLoopRegionNodeIds, compartmentEdges, compartmentsCompatible, detectSubFloInvokeCycle, listSubFloAnchors, loopHasExitEdge, loopHasLoopEdge, nodeSubFloId, } from '../utils/subFloGraph.js';
import { nodeLabel } from './bindings.js';
function validateGraphSubFloAndLoop(nodes, edges) {
    const issues = [];
    ...
    for (const anchor of anchors) {
        ...
        if (name)
            canvasNames.set(name, anchor.id);
        const compartment = nodes.filter(n => n.id === anchor.id || nodeSubFloId(n) === anchor.id);
    ...
    for (const n of nodes) {
        ...
        if (!loopHasLoopEdge(n.id, edges)) {
            issues.push({
                ...
                nodeLabel: nodeLabel(n.data, n.id),
    ...
    for (const n of nodes) {
        if (n.type !== NODE_TYPES.INVOKE_SUB_FLO)
            continue;
        const d = n.data;
        ...
        const inputArgs = anchor.data.inputArgs ?? [];
    ...
    for (const e of edges) {
        ...
        if (!compartmentsCompatible(src, tgt)) {
            issues.push({
                ...
                nodeLabel: nodeLabel(src.data, src.id),
...
function validateGraphTopology(nodes, edges) {
    ...
    const skipDisconnected = (n) => Boolean(nodeSubFloId(n))
        || loopRegion.has(n.id)

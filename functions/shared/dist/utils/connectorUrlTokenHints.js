...
export function resolveUrlTokenVendorProfile(connector) {
    const hay = `${connector?.label ?? ''} ${connector?.id ?? ''}`.toLowerCase();
    if (hay.includes('workday'))
        return 'workday';
    if (hay.includes('salesforce') || hay.includes('sfdc'))
        return 'salesforce';
    if (hay.includes('sap'))
        return 'sap';
    if (hay.includes('oracle'))
        return 'oracle';
    return 'generic';
}
...
export function getUrlTokenHint(profile, source, ordinal) {
    if (source === 'static') {
        return {
...
export function reassignUrlTokenKeys(tokens) {
    let n = 0;
    return tokens.map(t => {
        if (t.source === 'static')
            return t;
...
export function nextUrlTokenKey(tokens) {
    let max = 0;
...
export function sourceOrdinalBefore(tokens, index, source) {
    let n = 0;
...
export function createEmptyUrlToken(source, tokens, profile, insertIndex = tokens.length) {
    if (source === 'static') {
...
export function previewSampleForToken(profile, token, tokens) {
...
export function getUrlTokenHintForToken(profile, token, tokens) {
    const idx = tokens.indexOf(token);
    const primary = token.sources?.[0] ?? token.source;
    const ordinal = sourceOrdinalBefore(tokens, idx >= 0 ? idx : tokens.length, primary);

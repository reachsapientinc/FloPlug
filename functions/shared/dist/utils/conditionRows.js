 ...
 */
import { evaluateFilterCondition, } from './filterOperands.js';
export function computeGlobalParenBalance(rows) {
    let depth = 0;
    let totalOpen = 0;
    let totalClose = 0;
...
export function rowsUseGroupingColumns(rows) {
...
export function validateConditionRowsParentheses(rows) {
    if (!rows?.length)
        return { ok: true };
    let depth = 0;
...
function buildConditionTokens(rows, item, store, floRunMeta) {
    const tokens = [];
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        if (i > 0) {
...
function evaluateConditionTokens(tokens) {
    let i = 0;
    const parseTerm = () => {
        const t = tokens[i];
        if (!t)
        ...
        if (t.kind === 'paren' && t.value === '(') {
            ...
            if (tokens[i]?.kind === 'paren' && tokens[i].value === ')')
                i++;
            return inner;
        }
        if (t.kind === 'bool') {
...
export function evaluateConditionRows(rows, item, store, floRunMeta) {
    if (!rows?.length)
        return true;

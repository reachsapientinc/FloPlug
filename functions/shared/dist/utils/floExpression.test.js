...
group('replace / replaceAll / replaceN', () => {
    const e = (x) => evalExpression(x, ctx);
...
group('Nested expressions', () => {
    ...
    test('substring + length', () => e('substring(cStream.name, 0, length(cStream.name)-4)') === 'John');
    // substring after indexOf  →  "Doe"
    test('substring + indexOf', () => e("substring(cStream.name, indexOf(cStream.name, '_')+1)") === 'Doe');
    // pad(upper(trim(code)), 8, '0', 'l')
    test('pad + upper + trim', () => e("pad(upper(trim(cStream.code)), 8, '0', 'l')") === '00AB-001');
    // concat with format
    test('concat + format', () => e("concat(global.currency, ' ', format(cStream.salary, '#,##0.00'))")
        === 'USD 1,234,567.89');
    // iif with nested comparison using local
    test('iif + local var', () => e("iif(cStream.age >= local.threshold, 'adult', 'minor')") === 'adult');
    // coalesce with expression argument
    test('coalesce + exists', () => e("coalesce(cStream.missing, concat(cStream.firstName, ' (default)'))") === 'John (default)');
    // Arithmetic inside substring end index
    test('expr in substring bounds', () => e('substring(cStream.greeting, 6, 6 + length(cStream.firstName))') === 'world');
    // replaceAll inside concat
    test('concat + replaceAll', () => e("concat('[', replaceAll(cStream.tags, ',', ', '), ']')") === '[red, green, blue]');
});
// ─────────────────────────────────────────────────────────────────────────────
// looksLikeExpression
// ─────────────────────────────────────────────────────────────────────────────
...
group('safeEvalExpression', () => {
    test('bad expr returns fallback', () => safeEvalExpression('unknown_fn(x)', ctx, 'FALLBACK') === 'FALLBACK');
    test('good expr passes through', () => safeEvalExpression('cStream.age + 1', ctx, 0) === 26);
});
// ─────────────────────────────────────────────────────────────────────────────
// Summary
// ─────────────────────────────────────────────────────────────────────────────

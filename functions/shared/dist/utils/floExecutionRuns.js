...
export function formatFloRunVersion(floVersion, executedGraph) {
    if (executedGraph === 'draft')
        return 'draft';
    if (typeof floVersion === 'number' && floVersion > 0)
        return `v${floVersion}`;
    if (floVersion === 0)
        return 'draft';
    return '—';
...
export function sortRunsByStartedDesc(runs) {
...
export function collectRunVersionOptions(runs) {
    const keys = new Set();
    for (const r of runs) {
    ...
    const out = [{ value: '', label: 'All versions' }];
